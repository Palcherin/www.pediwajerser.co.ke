...
const formatOrderText = (order) => {
  const items = order.orderItems
    .map(
      (item, i) =>
        `  ${i + 1}. Product ID: ${item.productId} | Qty: ${item.quantity} | Price: KSh ${(item.price * item.quantity).toLocaleString()}`
    )
    .join('\n');
    ...
    .map(
      (item, i) => `
        <tr style="background:${i % 2 === 0 ? '#f9fafb' : '#ffffff'}">
          ...
          <td style="padding:10px 14px">Product ID: ${item.productId}</td>
...
export const sendWhatsApp = async (order) => {
  try {
    const encoded = encodeURIComponent(formatOrderText(order));
    const url = `https://api.callmebot.com/whatsapp.php?phone=${process.env.YOUR_WHATSAPP_NUMBER}&text=${encoded}&apikey=${process.env.CALLMEBOT_API_KEY}`;

    // Temporary debug — remove after fixing
    console.log('📲 Calling URL:', `https://api.callmebot.com/whatsapp.php?phone=${process.env.YOUR_WHATSAPP_NUMBER}&apikey=${process.env.CALLMEBOT_API_KEY}`);

    const res = await fetch(url);
    const body = await res.text();           // read the actual error message
...
export const sendEmail = async (order) => {
  try {
    await mailer.sendMail({
      from: `"Pedi Wa Jersey 🛒" <${process.env.EMAIL_USER}>`,
      to: process.env.EMAIL_USER,
      subject: `🛒 New Order — ${order.customerName} | KSh ${Number(order.totalAmount).toLocaleString()}`,
...
export const notifyAll = async (order) => {
  ...
  results.forEach((result, i) => {
    const channel = ['WhatsApp', 'Email'][i];
